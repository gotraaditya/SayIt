...
`--require-hashes`, `--no-cache-dir`, and `--no-build-isolation` after
installing the pinned release-tool and packaging locks, so source distributions
cannot fetch unpinned build tools or reuse machine-local wheels.
Release sidecar builds must also install PyInstaller and its
build-time dependencies from `python-backend\packaging-requirements.lock.txt`
with `--require-hashes`. The packaged spaCy language model must be installed
from `python-backend\model-requirements.lock.txt` with `--require-hashes` and
`--no-deps`; direct unverified wheel installation is not allowed. Release audit,
SBOM, and Python packaging bootstrap
...
The npm SBOM is generated by npm's built-in CycloneDX generator from
`package-lock.json`; no separately downloaded npm SBOM executable is allowed.
